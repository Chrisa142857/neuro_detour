...
batch_size = 32
device = 'cuda:2'
saver = 'baseline_NAGphormer-grouped_out'
...
for i in trange(len(xs), desc='convert to pyg data'):
    xyedge_datalist.append(Data(x=xs_tensor[i], new_feature=torch.from_numpy(xnagfeats[i]).to(device), adj_fc=torch.from_numpy(xadjfcs[i:i+1]).to(device), y=ys_tensor[i:i+1]))
