...
class TransformerModel(nn.Module):
    def __init__(
        ...
        **kargs
        ...
        self.out_proj = nn.Linear(self.hidden_dim, out_channel)
    ...
    def forward(self, data):
        ...
        return torch.relu(self.out_proj(output))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_bias=None):
        ...
        assert x.size() == orig_q_size, f"{x.size()} != {orig_q_size}"
        return x
...
class NAGdataTransform:
    def __init__(self, hops=7, **kargs):
        self.k = hops
    ...
    def __call__(self, data):
        ...
        nodes_features = torch.empty(nodesz, 1, self.k+1, features.shape[1])

        for i in range(nodesz):
