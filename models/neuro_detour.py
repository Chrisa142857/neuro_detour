...
class DetourTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        *args, **kwargs) -> None:
        ...
        self.lin_in = nn.Sequential(nn.Linear(in_channel, out_channel), nn.BatchNorm1d(out_channel), nn.LeakyReLU())
        self.concat = concat
        if concat:
    ...
    def forward(self, data):
        ...
        #     data.ID
        # )
        # extra_encodings = (data.DE, )
        # for ei, encoding in enumerate(extra_encodings):
        #     node_feature = node_feature + self.extra_encs[ei](encoding)
        ...
        #     node_feature = self.in_bn(node_feature.transpose(-1, -2)).transpose(-1, -2)
        node_feature = self.net(node_feature, mask=multi_mask)
        node_feature = self.in_bn(node_feature.transpose(-1, -2)).transpose(-1, -2)
