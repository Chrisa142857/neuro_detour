...
batch_size = 1000
device = 'cuda:1'
saver = 'ICD_NeuroDetour-grouped_out'
...
for i in trange(len(xs), desc='convert to pyg data'):
    xyedge_datalist.append(Data(
        x=xs_tensor[i], 
        adj_sc=torch.from_numpy(xadjscs[i:i+1]),#.to(device), 
        adj_fc=torch.from_numpy(xadjfcs[i:i+1]),#.to(device), 
        y=ys_tensor[i:i+1]))

val_batches = [Batch.from_data_list([xyedge_datalist[idxi] for idxi in val_idx[bi : bi+batch_size]]).to(device) for bi in trange(0, len(val_idx), batch_size, desc='prepare val batches')]
train_batches = []
...
class GraphNet(nn.Module):
    def __init__(self, in_channel, out_channel, hid_channel=512, **kwargs) -> None:
        super().__init__()
        self.node_sz = 116
        self.net = neuro_detour.DetourTransformer(node_sz=self.node_sz, in_channel=in_channel, out_channel=hid_channel, hiddim=hid_channel, nlayer=2, heads=4, batch_size=batch_size, device=device)
...
train_losses, val_losses = [None for _ in vocab_grouped], [None for _ in vocab_grouped]
patience = 0
best_epoch = 0

for mi in (pbar := tqdm((multiclass_labels.sum(0)*-1).argsort())):
    if vocab_grouped[mi] != 'Chapter-XVIII': continue
# for mi in (pbar := trange(len(vocab_grouped))):
    ...
    for epoch in range(1, epochs+1):
        ...
        if val_loss <= best_losses[mi]: 
            ...
            model.to(device)
            patience = 0
        log = f'{vocab_grouped[mi]} Train {train_loss:.5f} Val {val_loss:.5f} (Best {best_losses[mi]:.5f}@e{best_epoch:03d}) LR {scheduler.get_lr()[0]:.9f}'
