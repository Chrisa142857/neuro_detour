...
batch_size = 2000
device = 'cuda:2'
saver = 'ICD_NeuroDetourSingleD-beh_out'
...
class GraphNet(nn.Module):
    def __init__(self, in_channel, out_channel, hid_channel=1536, **kwargs) -> None:
        super().__init__()
        self.node_sz = 116
        self.net = neuro_detour.DetourTransformer(node_sz=self.node_sz, in_channel=in_channel, out_channel=hid_channel, hiddim=hid_channel, nlayer=2, heads=8, batch_size=batch_size, device=device)
    ...
    for foldi, (train_index, val_index) in enumerate(kf.split(all_subjects)):
        ...
        if no_train:
            ...
            for epoch in (pbar := trange(1, epochs+1)):
                ...
                log = f'Train {train_loss:.5f} Val {val_loss:.5f} (Best {best_loss:.5f}@e{best_epoch:03d}) LR {scheduler.get_lr()[0]:.9f}'
