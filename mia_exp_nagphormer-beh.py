...
batch_size = 32
device = 'cuda:5'
saver = 'baseline_NAGphormer-beh_out'
...
def multiclass_train(classifier, device, loader, optimizer, epoch):
    ...
    for batch in batches:
        ...
        loss = loss_fn(y.float(), gt.to(device))
        loss.backward()
        optimizer.step()
...
for tgt_task in all_tgt_tasks:
    ...
    for foldi, (train_index, val_index) in enumerate(kf.split(all_subjects)):
        ...
        for i in trange(len(xs), desc='convert to pyg data'):
            xyedge_datalist.append(Data(x=xs_tensor[i], new_feature=torch.from_numpy(xnagfeats[i]), adj_fc=torch.from_numpy(xadjfcs[i:i+1]), y=ys_tensor[i:i+1]))
