...
def get_multihop_path(bi, data, tgt_c, mat1, w2, node_sz, path_n):
    multihop_path = {}
    assert len(tgt_c) == 2, tgt_c
    for ci in tgt_c:
    # for ci in trange(mat1.shape[-1]):
        ...
        for h in range(heads):
            ...
            rowi = torch.where(headmat[:, ci]>0)[0].to(mat1.device)
            nodes = torch.LongTensor([ci]).repeat(len(rowi)).to(mat1.device) # node
            ...
            if len(pathways) == 0: 
                ...
                adjacent = data.adj_sc[bi, pathways[:, -1], curr_node]#.detach().cpu()
            ...
            if pathways.shape[1] > 2:
                fc_ind = torch.where(torch.logical_or(pathways[:, -1] == tgt_c[0], pathways[:, -1] == tgt_c[1]))[0]
                ...
                # print(path.shape)
                # exit()
                if f'{h+1}hop' not in multihop_path: 
                    multihop_path[f'{h+1}hop'] = []
                multihop_path[f'{h+1}hop'].append(path.detach().cpu())
    ...
    for i in torch.argsort(weights, descending=True):
        ...
        path_adj[paths[headi[i]][pathi[i], :-1], paths[headi[i]][pathi[i], 1:]] = path_i# (20-(path_i+1))/20#weights[i].repeat(paths[headi[i]].shape[1]-1)
...
model = VisDetourTransformer(node_sz=node_sz, in_channel=node_sz, out_channel=768, device=device)
...
if dname == 'adni':
    ...
    tgt_cs = [
        # [73, 87], [72, 86], [53, 59]
        [i, j] for i in range(node_sz) for j in range(i+1, node_sz)
    ]
elif dname == 'oasis':
...
for data in tqdm(tl):
    ...
    with torch.no_grad():
        attn_td, attn_fc = model(data.to(device))
    # fc = torch.stack(torch.tensor_split(data.x.cpu(), data.ptr.cpu())[1:-1])
    mat1 = attn_td#.detach().cpu()#.numpy()
    mat2 = attn_fc#.detach().cpu()#.numpy()
    ...
    for bi in range(bsz): 
        ...
        # print(f'Sub {subi} Label {data.y[bi].item()} Got {len(iso_paths)} paths', iso_paths)
        # tgt_cstr = "-".join([str(tgt_ci) for tgt_ci in tgt_c])
        # np.savetxt(f'resources/{atlas}_{dname}_top{path_n}Path_signFC{tgt_cstr}_{data.subject[bi]}_label{data.y[bi].item()}_adj.edge', path_adj.numpy(), delimiter='\t', fmt='%.5f')
        ########################
        for tgt_c in tqdm(tgt_cs):
            try:
                iso_paths, path_adj = get_multihop_path(bi, data, tgt_c, mat1, w2, node_sz, path_n)
                tgt_cstr = "-".join([str(tgt_ci) for tgt_ci in tgt_c])
                np.savetxt(f'resources_nimg/{atlas}_{dname}_top{path_n}Path_signFC{tgt_cstr}_{data.subject[bi]}_label{data.y[bi].item()}_adj.edge', path_adj.numpy(), delimiter='\t', fmt='%.5f')
